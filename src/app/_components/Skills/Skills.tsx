        ...
        >
          {data.map((skill) => {
            return (
              <div
                key={skill?.name}
                className="group relative flex h-24 w-24 cursor-pointer overflow-hidden rounded-full border-2 border-white transition-all duration-300 hover:border-[#e60000] md:h-40 md:w-40"
              >
                <motion.img
                  initial={{
                    opacity: 0,
                  ...
                  title={skill?.name}
                />
                <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white h-full w-full flex justify-center items-center scale-0 group-hover:scale-100 transition-all duration-200 rounded-full">
